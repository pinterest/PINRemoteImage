...
@implementation PINAPNGAnimatedImage
...
- (instancetype)initWithAnimatedImageData:(NSData *)animatedImageData
{
    if (self = [super init]) {
        ...
        if (_imageSource && [animatedImageData pin_isAPNG]) {
            ...
            _loopCount = (uint32_t)[[[imageProperties objectForKey:(__bridge NSString *)kCGImagePropertyPNGDictionary]
                                     objectForKey:(__bridge NSString *)kCGImagePropertyAPNGLoopCount] unsignedLongValue];
            ...
            for (NSUInteger frameIdx = 0; frameIdx < _frameCount; frameIdx++) {
                _durations[frameIdx] = [PINAPNGAnimatedImage frameDurationAtIndex:frameIdx source:_imageSource];
            }
        } else {
            return nil;
        }
...
+ (Float32)frameDurationAtIndex:(NSUInteger)index source:(CGImageSourceRef)source
{
    ...
    NSNumber *unclamedDelayTime = frameProperties[(__bridge NSString *)kCGImagePropertyPNGDictionary][(__bridge NSString *)kCGImagePropertyAPNGUnclampedDelayTime];
    if (unclamedDelayTime != nil) {
        frameDuration = [unclamedDelayTime floatValue];
    } else {
        NSNumber *delayTime = frameProperties[(__bridge NSString *)kCGImagePropertyPNGDictionary][(__bridge NSString *)kCGImagePropertyAPNGDelayTime];
        if (delayTime != nil) {
