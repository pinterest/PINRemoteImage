...
- (void)setProgressImageThresholds:(NSArray *)progressImageThresholds completion:(dispatch_block_t)completion
{
    __weak typeof(self) weakSelf = self;
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        typeof(self) strongSelf = weakSelf;
        [strongSelf lock];
        strongSelf.progressThresholds = progressImageThresholds;
        [strongSelf unlock];
        if (completion) {
            completion();
        }
...
- (void)didReceiveData:(NSData *)data forTask:(NSURLSessionDataTask *)dataTask
{
    ...
    NSMutableArray *downloadProgressBlocks = [[NSMutableArray alloc] init];
    ...
    for (PINRemoteImageCallbacks *completion in task.callbackBlocks.allValues) {
        ...
        if (downloadProgressBlock) {
            [downloadProgressBlocks addObject:downloadProgressBlock];
        }
    }
    
    PINProgressiveImage *progressiveImage = task.progressImage;
    BOOL hasProgressBlocks = task.hasProgressImageBlocks;
    [self unlock];
    ...
    for (PINRemoteImageManagerDownloadProgress downloadProgressBlock in downloadProgressBlocks) {
